...
// Pins the time-sorted invariant the karaoke display (LyricsView renders + seeks cues by array
// index) assumes: ♪ insertion must yield time-ordered cues even when the aligner emits a line out
// of order, so a lyric can't be stranded on the wrong side of an interlude.
@MainActor
final class SubtitleTimingOrderingTests: XCTestCase {
    ...
    func testVADMarkersSortSpeechBeforeInterleaving() {
        ...
        let out = SubtitleTimingTools.insertMusicMarkers(
    ...
    func testAlreadySortedCuesAreUnperturbed() {
        ...
        let vocalSegments = [(start: 0.0, end: 3.0), (start: 30.0, end: 31.0)]
        let out = SubtitleTimingTools.insertMusicMarkers(
            cues: sorted, durationMs: 32_000, vocalSegments: vocalSegments, minGapMs: 4000
        )
