...
public struct CTCForcedAligner {
    ...
    public func align(
        ...
        // Downloads the CTC model on first use; cached thereafter (in OS-purgeable Caches/, so it can
        // silently re-download if iOS evicted it). Surface the staged progress to the UI so this isn't
        // a frozen "Preparing alignment model…" through a ~400 MB download + weight load — and
        // breadcrumb the milestones (with availMem) so a run that dies here reveals WHETHER it died
        // mid-download (network) or mid-weight-load (memory). Skip the high-frequency download ticks.
        onStage?("Preparing alignment model…")
        Self.breadcrumb("aligner: fromPretrained begin")
        let aligner = try await Qwen3ForcedAligner.fromPretrained(progressHandler: { frac, stage in
