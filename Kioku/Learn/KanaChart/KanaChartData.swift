...
// Display data for the kana reference chart: gojūon, dakuten, and handakuten rows. Kept with the
// chart (not in the Dictionary `KanaData` segmentation/normalization enum) so UI reference data and
// segmentation constants stay separate concerns. Each `nil` entry is an empty cell in the grid.
nonisolated enum KanaChartData {
