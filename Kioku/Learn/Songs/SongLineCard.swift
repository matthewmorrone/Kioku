...
struct SongLineCard: View {
    ...
    private func wordHeadword(_ word: SongWord) -> some View {
        if let runReadings = wordFurigana[word.surface], runReadings.isEmpty == false {
            ...
            // width. Without it, the label reports the entire row as its size and its
            // internal .center paragraph alignment draws the headword centered in the row —
            // inconsistent with the plain-Text branch below, which already hugs its own
            // natural width and sits flush left.
            FuriganaLabel(
