...
extension ReadView {
    ...
    func initialMergedEdgeBounds(for selectedLocation: Int) -> ClosedRange<Int>? {
        guard let selectedIndex = document.segmentEdges.firstIndex(where: { edge in
    ...
    func adjacentSegmentSurfaces(for selectedLocation: Int) -> (left: String?, right: String?) {
        let activeBounds = segmentSelection.selectedBounds ?? initialMergedEdgeBounds(for: selectedLocation)
        ...
        if activeBounds.lowerBound > 0 {
            let leftEdge = document.segmentEdges[activeBounds.lowerBound - 1]
            let currentEdge = document.segmentEdges[activeBounds.lowerBound]
        ...
        if rightIndex < document.segmentEdges.count {
            let currentEdge = document.segmentEdges[activeBounds.upperBound]
            let rightEdge = document.segmentEdges[rightIndex]
    ...
    func mergeEdges(at edgeIndex: Int, isMergingLeft: Bool, updateSelection: Bool) -> Int? {
        guard document.segmentEdges.indices.contains(edgeIndex) else { return nil }

        let mergeBounds: ClosedRange<Int>
        if isMergingLeft {
            guard edgeIndex > 0 else { return nil }
            let leftEdge = document.segmentEdges[edgeIndex - 1]
            let rightEdge = document.segmentEdges[edgeIndex]
            ...
            guard edgeIndex + 1 < document.segmentEdges.count else { return nil }
            let leftEdge = document.segmentEdges[edgeIndex]
            let rightEdge = document.segmentEdges[edgeIndex + 1]
            guard isMergeAllowed(between: leftEdge, and: rightEdge) else {
                flashIllegalMergeBoundary(between: leftEdge, and: rightEdge)
                return nil
            }
        ...
        let sourceLeftSurface = document.segmentEdges[mergeBounds.lowerBound].surface
        let sourceRightSurface = document.segmentEdges[mergeBounds.upperBound].surface
        let mergedStart = document.segmentEdges[mergeBounds.lowerBound].start
        let mergedEnd = document.segmentEdges[mergeBounds.upperBound].end
        let mergedSurface = String(document.text[mergedStart..<mergedEnd])
        ...
        if updateSelection, document.segmentEdges.indices.contains(resultIndex) {
            let resolvedEdge = document.segmentEdges[resultIndex]
            segmentSelection.selectedBounds = resultIndex...resultIndex
            let mergedNSRange = NSRange(resolvedEdge.start..<resolvedEdge.end, in: document.text)
            segmentSelection.selectedSegmentLocation = mergedNSRange.location
            segmentSelection.selectedHighlightRangeOverride = mergedNSRange
        }
    ...
    func splitEdge(at edgeIndex: Int, offsetUTF16: Int, updateSelection: Bool) -> (leftIndex: Int, rightIndex: Int)? {
        ...
        let sourceEdge = document.segmentEdges[edgeIndex]
        let sourceSurface = sourceEdge.surface
        let sourceNSRange = NSRange(sourceEdge.start..<sourceEdge.end, in: document.text)
        ...
        guard
            let leftStringRange = Range(leftRange, in: document.text),
            let rightStringRange = Range(rightRange, in: document.text)
        else {
            return nil
        }

        let leftSurface = String(document.text[leftStringRange])
        let rightSurface = String(document.text[rightStringRange])
        ...
        if shouldApplyChangesGlobally {
            ...
            for (sourceIdx, edge) in updatedEdges.enumerated() {
                if edge.surface == sourceSurface {
                    ...
                    if let edgeLeftStringRange = Range(edgeLeftRange, in: document.text),
                       let edgeRightStringRange = Range(edgeRightRange, in: document.text) {
                        let edgeLeftSurface = String(document.text[edgeLeftStringRange])
                        let edgeRightSurface = String(document.text[edgeRightStringRange])
        ...
        if updateSelection, document.segmentEdges.indices.contains(resultLeftIndex) {
            let resolvedLeftEdge = document.segmentEdges[resultLeftIndex]
            let resolvedLeftNSRange = NSRange(resolvedLeftEdge.start..<resolvedLeftEdge.end, in: document.text)
            segmentSelection.selectedBounds = resultLeftIndex...resultLeftIndex
            segmentSelection.selectedSegmentLocation = resolvedLeftNSRange.location
            segmentSelection.selectedHighlightRangeOverride = resolvedLeftNSRange
        }
    ...
    func mergeAdjacentSegment(isMergingLeft: Bool) -> (surface: String, leftNeighborSurface: String?, rightNeighborSurface: String?)? {
        ...
        let mergedEdge = document.segmentEdges[mergedIndex]
        let leftNeighborSurface: String?
        if mergedIndex > 0, isMergeAllowed(between: document.segmentEdges[mergedIndex - 1], and: mergedEdge) {
            leftNeighborSurface = document.segmentEdges[mergedIndex - 1].surface
        } else {
            leftNeighborSurface = nil
        }
        let rightNeighborIndex = mergedIndex + 1
        let rightNeighborSurface: String?
        if rightNeighborIndex < document.segmentEdges.count,
           isMergeAllowed(between: mergedEdge, and: document.segmentEdges[rightNeighborIndex]) {
            rightNeighborSurface = document.segmentEdges[rightNeighborIndex].surface
        } else {
    ...
    func applySplitSelection(offsetUTF16: Int) -> (surface: String, leftNeighborSurface: String?, rightNeighborSurface: String?)? {
        guard let mergedRange = currentMergedSelectionNSRange(),
              let edgeIndex = document.segmentEdges.firstIndex(where: { edge in
                  let edgeRange = NSRange(edge.start..<edge.end, in: document.text)
        ...
        let leftEdge = document.segmentEdges[indices.leftIndex]
        let rightEdge = document.segmentEdges[indices.rightIndex]
        let leftNeighborSurface: String?
        if indices.leftIndex > 0,
           isMergeAllowed(between: document.segmentEdges[indices.leftIndex - 1], and: leftEdge) {
            leftNeighborSurface = document.segmentEdges[indices.leftIndex - 1].surface
        } else {
            leftNeighborSurface = nil
        }
        return (
    ...
    func currentMergedSelectionNSRange() -> NSRange? {
        if let selectedHighlightRangeOverride = segmentSelection.selectedHighlightRangeOverride,
        ...
        if let mergedBounds = segmentSelection.selectedBounds {
            let mergedStart = document.segmentEdges[mergedBounds.lowerBound].start
            let mergedEnd = document.segmentEdges[mergedBounds.upperBound].end
            let mergedNSRange = NSRange(mergedStart..<mergedEnd, in: document.text)
            if mergedNSRange.location != NSNotFound, mergedNSRange.length > 0 {
                return mergedNSRange
            }
        }

        guard let selectedSegmentLocation = segmentSelection.selectedSegmentLocation else {
            return nil
        }

        return document.segmentRanges.compactMap { segmentRange in
    ...
    func substring(for nsRange: NSRange) -> String? {
        guard
            ...
            nsRange.length > 0,
            let range = Range(nsRange, in: document.text)
        else {
            return nil
        }
