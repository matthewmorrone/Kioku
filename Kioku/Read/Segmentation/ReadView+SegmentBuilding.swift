...
extension ReadView {
    ...
    // Delegates to SegmentRangeRestoration so non-ReadView callers (e.g. SongStepperView) can
    // restore the same persisted readings without needing a ReadView instance.
    func normalizedSegmentRanges(_ segments: [SegmentRange]?, for sourceText: String) -> [SegmentRange]? {
        SegmentRangeRestoration.normalizedSegmentRanges(segments, for: sourceText)
    }
    ...
    // the surface cursor. Annotations are stored segment-relative and rebased here. Delegates
