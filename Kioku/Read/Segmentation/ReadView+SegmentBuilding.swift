...
extension ReadView {
    ...
    func normalizedSegmentRanges(_ segments: [SegmentRange]?, for sourceText: String) -> [SegmentRange]? {
        ...
        return migratedUnknownRunSegmentRanges(normalizedRanges, in: sourceText)
    }

    // Collapses stale persisted per-character unknown runs from older builds into the current
    // fallback shape so contiguous Latin text and numbers restore as one token on note load.
    private func migratedUnknownRunSegmentRanges(_ segments: [SegmentRange], in sourceText: String) -> [SegmentRange] {
        guard segments.isEmpty == false else {
            return segments
        }

        func mergeableUnknownGroup(for segment: SegmentRange) -> String? {
            guard (segment.furigana?.isEmpty ?? true) else {
                return nil
            }

            let startIndex = String.Index(utf16Offset: segment.start, in: sourceText)
            let endIndex = String.Index(utf16Offset: segment.end, in: sourceText)
            guard startIndex < endIndex else {
                return nil
            }

            let surface = String(sourceText[startIndex..<endIndex])
            guard segmenter.resolvesSurface(surface) == false else {
                return nil
            }

            guard let firstCharacter = surface.first,
                  let group = ScriptClassifier.unknownGrouping(for: firstCharacter),
                  group == "latin" || group == "number" else {
                return nil
            }

            for character in surface where ScriptClassifier.unknownGrouping(for: character) != group {
                return nil
            }
        ...
        var migrated: [SegmentRange] = []
        migrated.reserveCapacity(segments.count)

        var index = 0
        while index < segments.count {
            let segment = segments[index]
            guard let group = mergeableUnknownGroup(for: segment) else {
            ...
            while nextIndex < segments.count,
                  segments[nextIndex].start == mergedEnd,
                  mergeableUnknownGroup(for: segments[nextIndex]) == group {
                mergedEnd = segments[nextIndex].end
                nextIndex += 1
            }
            ...
            let mergedStartIndex = String.Index(utf16Offset: mergedStart, in: sourceText)
            let mergedEndIndex = String.Index(utf16Offset: mergedEnd, in: sourceText)
            migrated.append(
                SegmentRange(
                    start: mergedStart,
                    end: mergedEnd,
                    surface: String(sourceText[mergedStartIndex..<mergedEndIndex])
                )
            )
            index = nextIndex
        }
